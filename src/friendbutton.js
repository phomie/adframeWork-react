...
export default function Friendbutton(props){
    ...
    useEffect(() =>{
        const getCurrentStatus = async() =>{
            const response =await Axios.get('/api/friend-request/44'+id)
...
if(status===STATUS_REQUEST_MADE_BY_YOU){
    ...
    return(<button onClick={sendRequest('cancel')}>cancel friend request</button>)
}

if(status===STATUS_REQUEST_MADE_TO_YOU){

    return(<button onClick={sendRequest('accept')}>Accepted</button>)
}

if(status===STATUS_REQUEST_ACCEPTED){

    return(<button onClick={sendRequest('unfriend')}>Gohome Stupid</button>)
}
